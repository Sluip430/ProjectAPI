...
const {TMDB_TO_MOVIE_URL, API_KEY, TMDB_GENRES_URL, TMDB_LANGUAGE_URL} = require('../constraint/constraint');

const getFilmsToDB = async () => {
    for (let i = 3; i < 200; i++) {
        try {
            const response = await axios.get(`${TMDB_TO_MOVIE_URL}${i}?${API_KEY}`);
...
const getLanguageToDB = async () => {
    try {
        const response = await axios.get(`${TMDB_LANGUAGE_URL}`);
